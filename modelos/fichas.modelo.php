...
class ModeloFichas {
	...
	static public function mdlMostrarFichasFecha($tabla, $item, $valor) {
		...
		if ($item != null) {
			...
			$stmt = Conexion::conectarBDFicha()->prepare("SELECT f.id_ficha, f.tipo_ficha, l.fecha_muestra, f.busqueda_activa, pa.cod_asegurado, concat_ws(' ', pa.paterno, pa.materno, pa.nombre) AS nombre_completo, pa.nro_documento, pa.sexo, pa.fecha_nacimiento, l.resultado_laboratorio, l.fecha_resultado, f.estado_ficha FROM fichas f, pacientes_asegurados pa, laboratorios l WHERE f.id_ficha = pa.id_ficha AND f.id_ficha = l.id_ficha AND l.$item = :$item ORDER BY f.id_ficha DESC");
			...
			$stmt = Conexion::conectarBDFicha()->prepare("SELECT f.id_ficha, f.tipo_ficha, l.fecha_muestra, f.busqueda_activa, pa.cod_asegurado, concat_ws(' ', pa.paterno, pa.materno, pa.nombre) AS nombre_completo, pa.nro_documento, pa.sexo, pa.fecha_nacimiento, l.resultado_laboratorio, l.fecha_resultado, f.estado_ficha FROM fichas f, pacientes_asegurados pa, laboratorios l WHERE f.id_ficha = pa.id_ficha AND f.id_ficha = l.id_ficha ORDER BY f.id_ficha DESC ASC LIMIT 5000");
			...
			if ($stmt->execute()) {
			  ...
			  $stmt = $pdo->prepare("UPDATE pacientes_asegurados SET cod_asegurado = :cod_asegurado, cod_afiliado = :cod_afiliado, cod_empleador = :cod_empleador, nombre_empleador = :nombre_empleador, paterno = :paterno, materno = :materno, nombre = :nombre, sexo = :sexo, nro_documento = :nro_documento, fecha_nacimiento = :fecha_nacimiento, edad = :edad, id_departamento_paciente = :id_departamento_paciente, id_localidad_paciente = :id_localidad_paciente, id_pais_paciente = :id_pais_paciente, zona = :zona, calle = :calle, nro_calle = :nro_calle, telefono = :telefono, email = :email, nombre_apoderado = :nombre_apoderado, telefono_apoderado = :telefono_apoderado WHERE id_ficha = :id_ficha");
				...
				if ($stmt->execute()) {
				  ...
				  $stmt = $pdo->prepare("UPDATE ant_epidemiologicos SET ocupacion = :ocupacion, contacto_covid = :contacto_covid, fecha_contacto_covid = :fecha_contacto_covid, ant_vacuna = :ant_vacuna, fecha_dosis_vacuna = :fecha_dosis_vacuna, dosis_vacuna = :dosis_vacuna, proveedor_vacuna = :proveedor_vacuna, diagnosticado_covid = :diagnosticado_covid, fecha_diagnosticado_covid = :fecha_diagnosticado_covid, pais_infeccion = :pais_infeccion, departamento_infeccion = :departamento_infeccion, municipio_infeccion = :municipio_infeccion, localidad_infeccion = :localidad_infeccion WHERE  id_ficha = :id_ficha");
					...
					$stmt->bindParam(":ant_vacuna", $datos["ant_vacuna"], PDO::PARAM_STR);
					$stmt->bindParam(":fecha_dosis_vacuna", $datos["fecha_dosis_vacuna"], PDO::PARAM_STR);
					$stmt->bindParam(":dosis_vacuna", $datos["dosis_vacuna"], PDO::PARAM_STR);
					$stmt->bindParam(":proveedor_vacuna", $datos["proveedor_vacuna"], PDO::PARAM_STR);

					$stmt->bindParam(":diagnosticado_covid", $datos["diagnosticado_covid"], PDO::PARAM_STR);
					$stmt->bindParam(":fecha_diagnosticado_covid", $datos["fecha_diagnosticado_covid"], PDO::PARAM_STR);
					...
					$stmt->bindParam(":pais_infeccion", $datos["pais_infeccion"], PDO::PARAM_STR);
					$stmt->bindParam(":departamento_infeccion", $datos["departamento_infeccion"], PDO::PARAM_STR);
					$stmt->bindParam(":municipio_infeccion", $datos["municipio_infeccion"], PDO::PARAM_STR);
					$stmt->bindParam(":localidad_infeccion", $datos["localidad_infeccion"], PDO::PARAM_STR);
					...
					if ($stmt->execute()) {
					  ...
					  $stmt = $pdo->prepare("UPDATE datos_clinicos SET fecha_inicio_sintomas = :fecha_inicio_sintomas, malestares = :malestares, malestares_otros = :malestares_otros, estado_paciente = :estado_paciente, fecha_defuncion = :fecha_defuncion, diagnostico_clinico = :diagnostico_clinico WHERE id_ficha = :id_ficha");
						...
						if ($stmt->execute()) {
						  ...
						  $stmt = $pdo->prepare("UPDATE hospitalizaciones_aislamientos SET tipo_aislamiento = :tipo_aislamiento, fecha_aislamiento = :fecha_aislamiento, lugar_aislamiento = :lugar_aislamiento, fecha_internacion = :fecha_internacion, establecimiento_internacion = :establecimiento_internacion, ventilacion_mecanica = :ventilacion_mecanica, terapia_intensiva = :terapia_intensiva, fecha_ingreso_UTI = :fecha_ingreso_UTI WHERE id_ficha = :id_ficha");

							$stmt->bindParam(":tipo_aislamiento", $datos["tipo_aislamiento"], PDO::PARAM_STR);
							...
							if ($stmt->execute()) {
							  ...
							  $stmt = $pdo->prepare("UPDATE enfermedades_bases SET enf_estado = :enf_estado, enf_riesgo = :enf_riesgo, enf_riesgo_otros = :enf_riesgo_otros WHERE id_ficha = :id_ficha");
								...
								if ($stmt->execute()) {
								  ...
								  $stmt = $pdo->prepare("UPDATE laboratorios SET estado_muestra = :estado_muestra, no_toma_muestra = :no_toma_muestra, id_establecimiento = :id_establecimiento, tipo_muestra = :tipo_muestra, nombre_laboratorio = :nombre_laboratorio, fecha_muestra = :fecha_muestra, fecha_envio = :fecha_envio, responsable_muestra = :responsable_muestra, observaciones_muestra = :observaciones_muestra WHERE id_ficha = :id_ficha");
	...
	static public function mdlGuardarFichaControl($tabla, $datos) {
		...
		try {
			...
			if ($stmt->execute()) {
			    ...
			    $stmt = $pdo->prepare("UPDATE pacientes_asegurados SET cod_asegurado = :cod_asegurado, cod_afiliado = :cod_afiliado, cod_empleador = :cod_empleador, nombre_empleador = :nombre_empleador, paterno = :paterno, materno = :materno, nombre = :nombre, sexo = :sexo, nro_documento = :nro_documento, fecha_nacimiento = :fecha_nacimiento, edad = :edad, telefono = :telefono, email = :email WHERE id_ficha = :id_ficha");
				...
				if ($stmt->execute()) {
					...
					if ($stmt->execute()) {
					    ...
					    $stmt = $pdo->prepare("UPDATE laboratorios SET estado_muestra = :estado_muestra, no_toma_muestra = :no_toma_muestra, tipo_muestra = :tipo_muestra, fecha_muestra = :fecha_muestra, fecha_envio = :fecha_envio, responsable_muestra = :responsable_muestra WHERE id_ficha = :id_ficha");
	...
	static public function mdlEliminarFicha($id_ficha)	{

		$stmt = Conexion::conectarBDFicha()->prepare("DELETE f, ae, dc, eb, ha, l, pa, pn FROM fichas f LEFT JOIN ant_epidemiologicos ae ON f.id_ficha = ae.id_ficha LEFT JOIN datos_clinicos dc ON f.id_ficha = dc.id_ficha LEFT JOIN enfermedades_bases eb ON f.id_ficha = eb.id_ficha LEFT JOIN hospitalizaciones_aislamientos ha ON f.id_ficha = ha.id_ficha LEFT JOIN laboratorios l ON f.id_ficha = l.id_ficha LEFT JOIN pacientes_asegurados pa ON f.id_ficha = pa.id_ficha LEFT JOIN personas_notificadores pn ON f.id_ficha = pn.id_ficha WHERE f.id_ficha = :id_ficha");
